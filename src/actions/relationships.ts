...
export async function getRelationshipsByMember(memberId: string): Promise<Relationship[]> {
  const { data, error } = await supabase
    .from('relationships')
    .select('*')
...
export async function getSpouses(memberId: string): Promise<(FamilyMember & { relationship: Relationship })[]> {
  ...
  for (const rel of spouseRelations) {
    const spouseId = rel.person1_id === memberId ? rel.person2_id : rel.person1_id;
  ...
  return spouses.sort((a, b) => (a.relationship.marriage_order || 1) - (b.relationship.marriage_order || 1));
}
...
export async function getSiblings(memberId: string): Promise<FamilyMember[]> {
  ...
  const siblingIds = [...new Set(rels.map(r => r.person2_id))].filter(id => id !== memberId);
...
export async function endMarriage(relationshipId: string, divorceDate: string): Promise<void> {
  const db = createServerSupabase();
  const { error } = await db
    .from('relationships')
